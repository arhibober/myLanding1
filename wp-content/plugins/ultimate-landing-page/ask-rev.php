...
function plugulp_check_installation_date() {
    ...
    $nobug = "";
    $nobug = get_option('plugulp_hide_noticeSec');
...
function plugulp_display_admin_notice() {
    ...
    echo '<div style="width:65%; display: inline-block;" ><br> <h3><b>' . __( 'Install our new Landing Page Builder ', 'tss' ) . '</b></h3>';

    echo '<p style="font-size:16px;">' . __( ' <i> Tip :  </i> With PluginOps landing page builder you can build any landing page desing using 120+ free design blocks. So Install the plugin while its <b>Free!!</b> ', 'tss' ) . '</p>
...
function plugulp_set_no_bug() {
    ...
    if ( 1 == $nobug ) {
        ...
        add_option( 'plugulp_hide_noticeSec', TRUE );
    }
